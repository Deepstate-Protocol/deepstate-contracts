...
contract DeepstateV1MultiPoolHandler is Test {
    ...
    function _applyLeg(
        ...
        bool restAllowed = !params.noRest;
        if (remaining != 0 && restAllowed && params.epoch < expectedEpochs[poolIndex]) {
            bytes32 activeBook = ENGINE.bookId(params.token0, params.token1, expectedEpochs[poolIndex]);
            TopOrder memory beforeActiveMatch = _top(activeBook, !params.isBid);
            uint160 activeBaseFilled;
            uint256 activeQuoteAmount;
            (remaining, activeBaseFilled, activeQuoteAmount) =
                _matchModel(activeBook, params.isBid, _tick(params.order), remaining);
            baseFilled += activeBaseFilled;
            quoteAmount += activeQuoteAmount;
            TopOrder memory afterActiveMatch = _top(activeBook, !params.isBid);
            hookCount = _appendHookIfNeeded(
                expectedHooks, hookCount, poolIndex, activeBook, !params.isBid, beforeActiveMatch, afterActiveMatch
            );
        }
