...
contract CoverageHook {
    ...
    uint192 public lastOutgoingAmount;
    uint40 public lastIncomingNonce;

    function execute(bytes32 poolId, bytes32 bookId, address token, uint192 outgoingAmount, uint40 incomingNonce)
        external
    {
        calls++;
        lastPoolId = poolId;
...
contract CoverageEngineHarness is SinglePairEngineHarness {
    ...
    function takeTopOrderChangeForCoverage() external returns (uint192 outgoingAmount, uint40 incomingNonce) {
...
contract RadixMatchingEngineCoverageTest is Test {
    ...
    function testCoverage_InternalHarnessEntrypoints() public {
        ...
        (uint192 outgoingAmount, uint40 incomingNonce) = engine.takeTopOrderChangeForCoverage();
    ...
    function testCoverage_CancelAskLeftLeafAndAbsentDivergentOrders() public {
        ...
        bytes32 absentBid = _order(type(uint24).max, 1, MAX_ORDER_NONCE - 20);
        assertLt(
        ...
        bytes32 absentAsk = _order(type(uint24).max, 1, MAX_ORDER_NONCE - 21);
        assertLt(
        ...
        quote.mint(address(engine), type(uint24).max);
    ...
    function testCoverage_AskSubtreeAggregateAndRecursivePaths() public {
        ...
        emit AskMatched(_bookId(), bestAsk, 1);
        vm.expectEmit(false, false, false, true, address(engine));
        emit AskMatched(_bookId(), lowerAsk, 2);
        vm.expectEmit(false, false, false, true, address(engine));
        emit AskMatched(_bookId(), higherAsk, 1);
    ...
    function testCoverage_BidSubtreeAggregateAndRecursivePaths() public {
        ...
        vm.expectEmit(false, false, false, true, address(engine));
        emit BidMatched(_bookId(), bestBid, 1);
        vm.expectEmit(false, false, false, true, address(engine));
        emit BidMatched(_bookId(), higherBid, 2);
        vm.expectEmit(false, false, false, true, address(engine));
        emit BidMatched(_bookId(), lowerBid, 1);
    ...
    function testCoverage_ExactSamePriceOffSpineAggregateAskAndBid() public {
        ...
        bytes32 askAggregate = _branchFor(firstAsk, secondAsk);
        vm.expectEmit(false, false, false, true, address(engine));
        emit AskMatched(_bookId(), bestAsk, 1);
        vm.expectEmit(false, false, false, true, address(engine));
        emit AskMatched(_bookId(), askAggregate, 5);
        ...
        bytes32 bidAggregate = _branchFor(firstBid, secondBid);
        vm.expectEmit(false, false, false, true, address(engine));
        emit BidMatched(_bookId(), bestBid, 1);
        vm.expectEmit(false, false, false, true, address(engine));
        emit BidMatched(_bookId(), bidAggregate, 5);
    ...
    function testCoverage_ExactMixedPriceOffSpineAggregateAskAndBid() public {
        ...
        emit AskMatched(_bookId(), lowerAsk, 2);
        vm.expectEmit(false, false, false, true, address(engine));
        emit AskMatched(_bookId(), higherAsk, 3);
        ...
        emit BidMatched(_bookId(), higherBid, 2);
        vm.expectEmit(false, false, false, true, address(engine));
        emit BidMatched(_bookId(), lowerBid, 3);
    ...
    function testCoverage_StopsInsideOffSpineSubtrees() public {
        ...
        assertEq(engine.askRoot(), _branchFor(lowerAsk, higherAsk));
    ...
    function testCoverage_StopsInsideOffSpineBidSubtree() public {
        ...
        assertEq(engine.bidRoot(), _branchFor(higherBid, lowerBid));
    ...
    function testCoverage_DirtyRightSpinesMaterializeOnSameSideInsert() public {
        ...
        bytes32 askAnchor = _branchFor(firstAsk, secondAsk);
    ...
    function testCoverage_DirtyBidRightSpineMaterializesOnBidInsert() public {
        ...
        bytes32 bidAnchor = _branchFor(firstBid, secondBid);
    ...
    function testCoverage_DirtyMixedPriceRightSpinesUseRecursiveRouting() public {
        ...
        vm.expectEmit(false, false, false, true, address(engine));
        emit AskMatched(_bookId(), _order(60, 1, _nonce(firstAsk)), 1);
        vm.expectEmit(false, false, false, true, address(engine));
        emit AskMatched(_bookId(), secondAsk, 3);
        ...
        vm.expectEmit(false, false, false, true, address(engine));
        emit BidMatched(_bookId(), _order(80, 1, _nonce(firstBid)), 1);
        vm.expectEmit(false, false, false, true, address(engine));
        emit BidMatched(_bookId(), secondBid, 3);
